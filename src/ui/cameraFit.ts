...
export function fitWidthCamera(viewWidth: number): Camera {
  const zoom = clampZoom((Math.max(120, viewWidth) - 48) / PDF_PAGE_FRAME.width);
  return { x: 20, y: 16, zoom };
}

export function fitPageCamera(viewWidth: number, viewHeight: number): Camera {
  const zoom = clampZoom(
    Math.min(
      (Math.max(120, viewWidth) - 48) / PDF_PAGE_FRAME.width,
      (Math.max(120, viewHeight) - 48) / PDF_PAGE_FRAME.height,
    ),
  );
  return { x: 20, y: 16, zoom };
}
